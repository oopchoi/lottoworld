...
//        };
        ...
        //for(int i=0; i<1; i++){
            ...
            for(int j=0; j<input[i].length-4; j++){
                Arrays.sort(input[i]);
                for(int m=j+1; m<input[i].length-3; m++){
                    for(int n=m+1; n<input[i].length-2; n++){
                        for (int k=n+1; k<input[i].length-1; k++) {
                            for (int x=k+1; x<input[i].length; x++) {
    ...
    private static void insertCombo(ArrayList completeCombo){
        try {
            ...
            String query = "INSERT INTO tbl_fix5_combination (num_1, num_2, num_3, num_4, num_5) VALUES (?, ?, ?, ?, ?)";

            PreparedStatement preparedStmt = conn.prepareStatement(query);

            PreparedStatement truncateStmt = conn.prepareStatement("truncate tbl_fix5_combination");
